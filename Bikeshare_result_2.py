...
def show_data(df):
    """Displays 5 rows of the data set if the user wants to."""
    start_loc = 0 # start with row 0
    show_data= input('Would you like to see the first 5 rows of data? Type yes or no.\n')
    while True: 
        if show_data.lower() != 'yes':
            break
        print(df.iloc[start_loc:start_loc+5]) # select 5 rows starting from start_loc
        start_loc += 5
        show_data= input('Would you like to see 5 more rows? Type yes or no.\n')
